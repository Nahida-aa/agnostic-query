...
describe('toDrizzleWhere', () => {
	it('=', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."name" = $1`,
		);
	...
	it('>', () => {
		const sql = toSql(
			toDrizzleWhere(users, { field: ['age'], op: '>', value: 18 }),
		);
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."age" > $1`,
		);
	...
	it('like', () => {
		const sql = toSql(
			toDrizzleWhere(users, { field: ['name'], op: 'like', value: '%test%' }),
		);
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."name" like $1`,
		);
		expect(sql.params).toEqual(['%test%']);
	});

	it('ilike', () => {
		const sql = toSql(
			toDrizzleWhere(users, { field: ['name'], op: 'ilike', value: '%Test%' }),
		);
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."name" ilike $1`,
		);
		expect(sql.params).toEqual(['%Test%']);
	});
	...
	it('is null', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."name" is null`,
		);
	...
	it('@> (contains)', () => {
		const sql = toSql(
			toDrizzleWhere(users, { field: ['tags'], op: '@>', value: ['admin'] }),
		);
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."tags" @> $1`,
		);
		expect(sql.params).toEqual(['{"admin"}']);
	});
	...
	it('<@ (contained by)', () => {
		...
		expect(sql.params).toEqual(['{"admin","user"}']);
	});
	...
	it('in', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where "users"."id" in ($1, $2, $3)`,
		);
	...
	it('and', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where ("users"."name" = $1 and "users"."age" > $2)`,
		);
	...
	it('or', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where ("users"."name" = $1 or "users"."name" = $2)`,
		);
	...
	it('not', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where not "users"."age" < $1`,
		);
		expect(sql.params).toEqual([18]);
	});
	...
	it('nested and/or/not', () => {
		...
		expect(sql.sql).toBe(
			`select "id", "name", "age", "tags", "data" from "users" where (("users"."name" like $1 or not "users"."age" = $2) and "users"."id" in ($3, $4))`,
		);
		expect(sql.params).toEqual(['%test%', 0, 'a', 'b']);
	});
	...
	it('returns undefined for non-existent column', () => {
		expect(
			toDrizzleWhere(users, { field: ['unknown'], op: '=', value: 'x' } as any),
		).toBeUndefined();
	});
